...
export const COUNTRY_CODES = [
  ...
  { code: "+234", country: "Nigeria", flag: "🇳🇬" }, // TESTING — remove before production
...
export function normalizePhoneBody(prefix: string, raw: string): string {
  ...
  if ((prefix === "+44" || prefix === "+234") && body.startsWith("0")) {
    body = body.slice(1);
  }
  return body;
}
...
export function validatePhoneForConfirmation(prefix: string, raw: string): string | null {
  ...
  if (prefix === "+234") {
    if (!/^[789]\d{9}$/.test(body)) {
      return "Enter a valid Nigerian mobile number (10 digits, e.g. 8012345678).";
    }
    return null;
  }
...
export function phonePlaceholder(prefix: string): string {
  if (prefix === "+234") return "8012345678";
